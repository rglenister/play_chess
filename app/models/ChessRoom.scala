...
class ChessRoom extends Actor {
  ...
  def receive = {
    ...
    case MakeMove(username, fromSquare, toSquare, promotionPiece) => {
      ...
      DB.update(DB.get.makeMove(fromSquare.toInt, toSquare.toInt, promotionPiece).getOrElse(DB.get))
      notifyAll("game", username, "")
    }
    
    case SetCurrentPosition(username, index) => {
      Logger.info("ChessRoom.receive SetCurrentPosition index=" + index)
      DB.update(DB.get.setCurrentPosition(index).getOrElse(DB.get))
      notifyAll("game", username, "")      
    }
  ...
  def notifyAll(kind: String, user: String, text: String) {
    val msg = JsObject(
      Seq(
        ...
        "game" -> JsonGameSerializer.serializeFrontEnd(DB.get),
        "members" -> JsArray(
